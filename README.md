...
- 設定 →「表示」→「エディタの白黒反転」で、黒背景に白文字になります。
...
- 編集: `x`、`dd`、`D`、`yy`、`p` `P`。コピーした内容はSDカード(`/.crosspoint/edit/yank.bin`)に置くので、RAMを使わず、電源を切っても残ります
- ビジュアルモード: `v`(文字)、`V`(行)、`Ctrl-V`(矩形)。モーションで範囲を広げ、`d`/`x` 削除、`y` コピー、`c` 削除して入力、`o` 反対の端へ、Escで抜ける。矢印キーも使えます
- 矩形: 列は表示セル単位(全角は2セル)で、`j`/`k` は論理行単位で動きます。`I`/`i` と `A`/`a` で、1行目に打った文字をEscで全部の行に入れます(日本語もOK)。コピーした矩形は `p` で縦に貼り付け
...
- 未対応: `dw` `cw` のようなオペレーターとモーションの組み合わせ
